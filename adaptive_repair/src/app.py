...
def analyze_sync(request_data: AnalyzeRequest):
    ...
    try:
        ...
        initial_state = {
            ...
            "agent_queue": [],
            "history": [],
            "user_feedback": request_data.user_feedback, # Incorporate Feedback from AnalyzeRequest
        }
...
def repair_sync(request_data: RepairRequest):
    ...
    try:
            ...
            # If language is empty, use placeholder "Auto" which will be detected by MainAgent
            effective_language = src_language if src_language and src_language.strip() else "Auto"
            
            initial_state = {
            "bug_id": request_id,
            "code": original_code,
            "src_lang": effective_language,
            ...
            "agent_queue": [],
            "history": [],
            "user_feedback": request_data.user_feedback, # Incorporate Feedback from RepairRequest
            "generate_tests": True, # Enable tests for web app
        }
            ...
            if entry_point == "main_node":
                ...
                if analysis_entry:
                    ...
                    # IMPORTANT: Use the agent's detected language instead of user-provided language
                    # This fixes issues where users select the wrong language in the UI
                    detected_lang = plan_from_analysis.get("detected_language")
                    if detected_lang:
                        # Update src_lang in final_state to use detected language
                        final_state["src_lang"] = detected_lang
                        logger.info(f"Using detected language '{detected_lang}' (user provided: '{src_language}')")
            ...
            # 5. Get test validation from graph state (generated by test_generator_node)
            test_validation = final_state.get("test_validation")
            ...
            "test_validation": test_validation,  # TestGeneratorAgent output (from graph)
        }
